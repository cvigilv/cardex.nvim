...
local S = require("cardex.helpers.string")
local U = require("cardex.utils")
local W = require("cardex.helpers.windows")

local M = {}

-- TODO: see what to do with this
local namespace = vim.api.nvim_create_namespace("cardex.ui")
...
M.show_links = function(note, opts)
  ...
  -- TODO: Test changing file path with tags just after title (sort tags by count in Cardex)
  ...
  vim.cmd([[syntax match CardexFromLinks /^ F .*/ |
      syntax match CardexBothLinks /^ B .*/ |
      syntax match CardexToLinks /^ T .*/ |
      highlight link CardexFromLinks Removed |
      highlight link CardexBothLinks Changed |
      highlight link CardexToLinks Added]])
