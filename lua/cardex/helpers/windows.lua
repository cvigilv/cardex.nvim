...
function M.new_floating_win(contents, opts)
  local defaults = {
    height = function(editor_height)
      return editor_height - 8
    end,
    width = function(editor_width)
      return math.max(96, math.ceil(editor_width * 0.45))
    end,
    row = 3,
    col = -2,
    border = { "🭽", "▔", "🭾", "▕", "🭿", "▁", "🭼", "▏" },
